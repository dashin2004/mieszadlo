define([
    ...
    'system/widgets/common/changePassword/Validator',
    'system/widgets/common/changePassword/Message',
    'system/widgets/ChangePasswordDialog/libs/Elements',
    'system/widgets/ChangePasswordDialog/libs/PolicyView'
], function (SuperClass, popUpManager, zoomManager, pageController, uiController, internalDispatcher, BreaseEvent, Enum, breaseConfig, Utils, SimpleQueue, userService, languageService, WindowType, Validator, Message, Elements, PolicyView) {
    ...
    */
    var defaultSettings = {
            ...
            windowType: WindowType.CHANGEPASSWORDDIALOG
        },
    ...
    * @param {Boolean} showPolicy shows or hides the policies in the dialog, is only considered if window width is big enough to display the policies
    * @param {Object} [policies] password policies when dialog is opened from forced login screen
    * @param {String} [headerTextKey] header when dialog is opened from foced login screen
    */
    p.show = function (userName, showPolicy, policies, headerTextKey) {

        userName = Utils.isString(userName) ? userName : '';
        showPolicy = Utils.isBoolean(showPolicy) ? showPolicy : false;
        showPolicy &&= _fitsOnDisplay();
        ...
        let options = {
            header: {
                text: _getHeaderText(userName, headerTextKey)
            }
        };
        SuperClass.prototype.show.call(this, options);

        if (userName) {
            uiController.setWidgetPropertyIndependentOfState(this.inputs.userName.elemId, 'value', userName);
            uiController.setWidgetPropertyIndependentOfState(this.inputs.userName.elemId, 'visible', false);
            uiController.setWidgetPropertyIndependentOfState(this.labels.userName.elemId, 'visible', false);
        }
        if (this.internalData.showPolicy) {
            let grpBoxPos = $('#chpwd_GroupBox').position();
            let btnChangePos = this.buttons.change.$el.position();
            let oldPasswordPos = this.inputs.oldPassword.$el.position();
        ...
        if (breaseConfig.isKeyboardOperationEnabled()) {
            this._initFocus(!userName);
        }
    };

    p._initFocus = function (hasUserInput) {
        if (hasUserInput) {
            this.inputs.userName.$el.get(0).focus();
        } else {
            this.inputs.oldPassword.$el.get(0).focus();
        }
    };

    p.loadPolicies = function () {
        var widget = this;
        userService.loadPasswordPolicies().then(
            function (policies) {
    ...
    p.getForm = function () {
        return {
            userName: uiController.callWidget(this.inputs.userName.elemId, 'getValue'),
            oldPassword: uiController.callWidget(this.inputs.oldPassword.elemId, 'getValue'),
            newPassword: uiController.callWidget(this.inputs.newPassword.elemId, 'getValue'),
            confirmPassword: uiController.callWidget(this.inputs.confirmPassword.elemId, 'getValue')
        };
    };
    ...
    p.setInputLabel = function (elemId, value) {
        ...
        if (value) {
            uiController.callWidget(this.labels[fieldName].elemId, 'setText', this.labels[fieldName].textKey);
        } else {
            uiController.callWidget(this.labels[fieldName].elemId, 'setText', '');
        }
    };
    ...
    p.sendChange = function (form) {
        var widget = this;
        return userService.changePassword(form.userName, form.oldPassword, form.newPassword).then(
    ...
    */
    p.hide = function () {
        ...
        internalDispatcher.dispatchEvent(new CustomEvent(BreaseEvent.CHANGEPASSWORDDIALOG_CLOSED, {
            detail: {
    ...
    p.dispose = function () {
        ...
        this.elem.removeEventListener(BreaseEvent.WIDGET_READY, this._getBound(_readyListener));
        this.elem.removeEventListener(BreaseEvent.CONTENT_PARSED, this._getBound(_parsedListener));
    ...
    /**
     * Overwrite in derived widget.
     * @returns {String} html template of widget
     */
    p.getTemplate = function () {
        return WidgetClass.template;
    };

    function _fitsOnDisplay() {
        return popUpManager.getDimensions().winWidth > 660;
    }

    function _loadHTML() {
        this.deferredInit(document.body, this.getTemplate());
        _checkParsed.call(this);
        _checkReady.call(this);
        uiController.parse(this.elem);
    }
    ...
    function _showResponse(type, response, userName) {
        ...
        if (type === 'error') {
            ...
            uiController.callWidget(this.buttons.confirm.elemId, 'setVisible', true);
            uiController.callWidget(this.buttons.confirm.elemId, 'focus');
    ...
    function _getHeaderText(userName, key) {
        let text = languageService.getText(key, true); // check if new text exists and fallback to previous one in case of update from V5.x
        
        if (key && text !== breaseConfig.undefinedTextReturnValue) {
            return text.replace('{1}', userName);
        }
        return (userName) ? _text('IAT/System/Dialog/CHANGEPASSWORD_DIALOG_HEADER_USERNAME').replace('{1}', userName) : _text('IAT/System/Dialog/CHANGEPASSWORD_DIALOG_HEADER');
    }
    var _errorKeys = {
