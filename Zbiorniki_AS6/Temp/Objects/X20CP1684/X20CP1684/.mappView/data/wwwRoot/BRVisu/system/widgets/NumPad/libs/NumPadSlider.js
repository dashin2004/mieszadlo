...
function (BreaseEvent, numberFormatter, Utils, Types, DOM, EnumObject, EventDispatcher, d3) {

    'use strict';

    /**
     * @class system.widgets.NumPad.libs.NumPadSlider
    ...
    /**
        ...
        * @param {NumberFormat} config.numberFormat
        * @return {Number[]}
        */
    ...
    function _calcTickValue(nr, config) {
        return numberFormatter.parseFloat(numberFormatter.formatNumber(nr, config.numberFormat, false));
    }
    ...
    NumPadSlider.prototype.setConfig = function (config) {
        ...
        this.tickFormatter = d => numberFormatter.formatNumber(d, this.settings.numberFormat, this.settings.useDigitGrouping);
    };
    ...
    function addMajorTickAxis(majorTickValues, config) {
        if (this.settings.showTicks === true || this.settings.showTickNumbers === true) {
            ...
            this.majorTickAxis = d3.svg.axis()
                ...
                .tickFormat(d => this.tickFormatter(d))
                .orient('top');
    ...
    function onMouseDown() {
        ...
        DOM.bodyEl.on(BreaseEvent.MOUSE_MOVE, this.bound_onMouseMove);
        DOM.bodyEl.on(BreaseEvent.MOUSE_UP, this.bound_onMouseUp);
    }
    ...
    function onMouseUp(e) {
        ...
        DOM.bodyEl.off(BreaseEvent.MOUSE_MOVE);
        DOM.bodyEl.off(BreaseEvent.MOUSE_UP);
        moveSlider.call(this, e);
    }

    function getZoomFactor() { // A&P 451830: Slider in Numpad not usable when zoomed
        var zoomFactor = DOM.bodyEl.css('zoom');
