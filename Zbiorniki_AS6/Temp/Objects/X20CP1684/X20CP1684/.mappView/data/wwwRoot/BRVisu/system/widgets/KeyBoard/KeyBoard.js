define(['system/widgets/Window/Window',
    'brease/controller/PopUpManager',
    'brease/events/BreaseEvent',
    'brease/config',
    'system/widgets/common/keyboards/FocusUtils',
...
function (SuperClass, PopUpManager, BreaseEvent, breaseConfig, FocusUtils, NodeInfo, Docking, WindowType, KeyboardType) {
    ...
    */
    ...
    p.init = function () {
        ...
        this.settings.windowType = WindowType.KEYBOARD;
    ...
    p.show = function (options, refElement) {
        ...
        if (!this.inputEl) {
            ...
            this.inputElem = this.inputEl.get(0);
        }
    ...
    p.inputFocus = function () {
        FocusUtils.focusInputField(this.inputEl);
    };
    ...
    p._keyDownHandler = function (e) {
        if (breaseConfig.visu.keyboardOperation && e.key === 'Escape') {
            this.hide();
        }
    };
    ...
    function removeListeners() {
        document.body.removeEventListener(BreaseEvent.DIALOG_OPEN, this._getBound(dialogOpenHandler));
        document.body.removeEventListener(BreaseEvent.CLOSED, this._getBound(overlayCloseHandler));
        document.body.removeEventListener(BreaseEvent.WINDOW_SHOW, this._getBound(overlayOpenHandler));
        document.body.removeEventListener('keydown', this._getBound('_keyDownHandler'));
    }
