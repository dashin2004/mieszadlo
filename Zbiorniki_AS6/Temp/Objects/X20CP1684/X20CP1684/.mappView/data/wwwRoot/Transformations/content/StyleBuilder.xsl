...
<!--
// COPYRIGHT B&R
// Stylesheet to transform XML Content definition into SCSS
// Generates id-selectors for all widgets in a content
// used in Content Editor only
// for AS Build a JavaScript replacement is used (jsBuild/libs/ScssTransformation.js)
-->
...
@use "sass:string";
  ...
  <xsl:template name="widget">
    ...
    <xsl:variable name="widgetFile" select="document($pathToWidgetFile)" />
    <xsl:variable name="isCompound" select="$widgetFile//widget:ASEngineeringInfo/widget:IsCompound/text()"></xsl:variable>
    <xsl:variable name="widgetFileWidgetElement" select="$widgetFile/widget:WidgetLibrary/widget:Widget"></xsl:variable>
