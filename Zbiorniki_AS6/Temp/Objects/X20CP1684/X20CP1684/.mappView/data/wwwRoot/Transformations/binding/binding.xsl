<xsl:stylesheet version="1.0"
                ...
                exclude-result-prefixes="bdg be blt wdg widgetList">
  ...
  <!-- File which contains a list of used widgets with their name and path-->
  ...
  <xsl:template match="bdg:Bindings/bdg:ComplexBinding">
    ...
    <Content id="{bdg:TargetRoot/@contentRefId}">
      ...
      <xsl:variable name="widgetType" select="@widgetType"/>
      <xsl:variable name="widgetFilePath">
      ...
      <xsl:variable name="bindingtemplate" select="document($widgetFilePath)//wdg:BindingTemplate[@id=$templateId]" />
      <xsl:if test="not($bindingtemplate/@id)">
        <xsl:message terminate="yes">
          Error: Defined template not found; File: <xsl:value-of select="$widgetFilePath" />; Widget: <xsl:value-of select="$widgetType" />; Template: <xsl:value-of select="$templateId" />
  ...
  <!--template function to get the path to the widget folder out of index file (widgetIndexPath) -->
  <xsl:template name="getWidgetFile">
    <!-- widgetType from ComplexBinding, e.g. widget.automationtest.Tank -->
    <xsl:param name="widgetType"></xsl:param>
    <xsl:variable name="widgetFolderPath">
     <xsl:value-of select="document($widgetIndexPath)//widgetList:widgets/widgetList:widget[@name=$widgetType]/@path" />
    </xsl:variable>
    <xsl:variable name="widgetName">
     ...
     <xsl:value-of select="concat($widgetFolderPath,$elpathdelimiter,'meta',$elpathdelimiter,$widgetName,'.widget')" />
  </xsl:template>
