define([
    ...
    'system/widgets/Window/libs/Config',
    'system/widgets/Window/libs/DragHandler',
    ...
    BreaseEvent, Enum, Utils, DOM, breaseSettings, languageService, languageDependency, popupManager, pageController, uiController, fileManager
) {

    'use strict';

    /**
    * @class system.widgets.Window
    ...
    */
    ...
    p.init = function () {
        ...
        this.settings.stylePrefix = this.settings.stylePrefix || fileManager.getPathByClass(this.settings.className, 'style');
        ...
        this.settings.parentContentId = breaseSettings.globalContent;
    ...
    */
    p.show = function (options, refElement) {
        ...
        popupManager.addWindow(this.elem.id, this.settings.windowType);
        ...
        if (this.settings.header !== undefined && languageService.isKey(this.settings.header.text)) {
            this.settings.header.textkey = languageService.parseKey(this.settings.header.text);
        }
        if (this.settings.content !== undefined && languageService.isKey(this.settings.content.text)) {
            this.settings.content.textkey = languageService.parseKey(this.settings.content.text);
        }
        ...
        DOM.bodyEl.on(BreaseEvent.CLICK + ' ' + BreaseEvent.DISABLED_CLICK, this._bind('_documentClickHandler'));
    ...
    p.closeOnLostContent = function (refElement) {
        this.opener = pageController.getLoaderForElement(refElement);
        if (this.opener) {
    ...
    p.removeCloseOnLostContent = function () {
        if (this.opener) {
            this.opener.removeEventListener(BreaseEvent.FRAGMENT_HIDE, this._getBound('closeOnLostContentListener'));
    ...
    */
    p.hide = function () {
        ...
        this.resetStyles();
        DOM.bodyEl.off(BreaseEvent.CLICK + ' ' + BreaseEvent.DISABLED_CLICK, this._bind('_documentClickHandler'));
        this.removeCloseOnLostContent();
        this._removeModal();
        popupManager.removeWindow(this.elem.id);
    ...
    function _removeModalListeners(widget) {
        ...
        widget.dimmer[0].removeEventListener('pointerdown', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].removeEventListener('pointerup', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].removeEventListener('touchstart', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].removeEventListener('touchend', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].removeEventListener('mousedown', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].removeEventListener('mouseup', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer.off(BreaseEvent.CLICK, widget._bind('_dimmerClickHandler'));
    }

    function _addModalListeners(widget) {
        var boundStopPropagation = widget._bind(_stopPropagation);
        widget.dimmer[0].addEventListener('pointerdown', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].addEventListener('pointerup', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].addEventListener('touchstart', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].addEventListener('touchend', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].addEventListener('mousedown', boundStopPropagation, { capture: true, passive: true });
        widget.dimmer[0].addEventListener('mouseup', boundStopPropagation, { capture: true, passive: true });
    ...
    function setArrowStyle(arrowPos, elTop, elLeft, elWidth, elHeight, refOffset, refWidth, refHeight) {
        var css = {},
            rootZoom = pageController.getRootZoom();
    ...
    p._setContent = function () {
        var headerText = (this.settings.header) ? ((this.settings.header.textkey) ? languageService.getText(this.settings.header.textkey) : this.settings.header.text) : '';
        if (this.headerEl === undefined) {
            this.headerEl = this.el.find('header > div');
        }
        this.headerEl.text(headerText);

        var contentText = (this.settings.content) ? ((this.settings.content.textkey) ? languageService.getText(this.settings.content.textkey) : this.settings.content.text) : '';
    ...
    p._setPosition = function () {
        ...
        if (!this.settings.position.hasOwnProperty('horizontalDialog')) {
            ...
            var rootZoom = pageController.getRootZoom();
            this.el.css({
                display: 'block',
                position: 'fixed',
                left: left + 'px',
    ...
    function _addCloseListener() {
        this._addCloseListenerTimeout = window.setTimeout(widget => {
            if (!widget.isDisposed()) {
                widget.closeButton.on(BreaseEvent.CLICK, widget._bind('_closeButtonClickhandler'));
            }
        }, 0, this);
    }
    ...
    p.readyHandler = function () {
        uiController.addWidget(this);
        this.readyDeferred.resolve(this);
        this._dispatchReady();
    };

    function _calcPositionWithArrow(position, settings, elWidth, elHeight, refOffset, refWidth, refHeight) {
        ...
        return _addArrowOffset(position, settings);
    }
    
    function _calcPositionWithoutArrow(position, settings, elWidth, elHeight, refOffset, refWidth, refHeight) {
        const rootZoom = pageController.getRootZoom();
    ...
    function _calcPosition(settings, elWidth, elHeight, refOffset, refWidth, refHeight, hasArrow) {
        ...
        if (hasArrow === true) {
            position = _calcPositionWithArrow(position, settings, elWidth, elHeight, refOffset, refWidth, refHeight);
        } else {
            position = _calcPositionWithoutArrow(position, settings, elWidth, elHeight, refOffset, refWidth, refHeight);
        }
        return position;
    }

    function _calcXAtTarget(position, settings, elWidth, refOffset, refWidth) {
    ...
    function _calcYAtTarget(position, settings, elHeight, refOffset, refHeight) {
    ...
    function _calcPositionAtTarget(settings, elWidth, elHeight, refOffset, refWidth, refHeight) {
        ...
        _calcXAtTarget(position, settings, elWidth, refOffset, refWidth);
        _calcYAtTarget(position, settings, elHeight, refOffset, refHeight);
        return position;
    }
    ...
    function _respectBoundaries(position, elWidth, elHeight, elemExtension, pointOfOrigin) {
        var globalDim = popupManager.getDimensions(),
            appDim = DOM.appElem.getBoundingClientRect(),
    ...
    function _loadHTML(widget) {
        let elemId = widget.settings.id || Utils.uniqueID('window');
        let html = WidgetClass.template.replace('WIDGETID', elemId);
        widget.deferredInit(document.body, html);
        widget.headerEl = widget.el.find('header > div');
        widget.readyHandler();
        uiController.parse(widget.elem);
    }
    ...
    function _disableScroll() {
        ...
        if (!_safe) {
            _safe = {
                x: DOM.bodyEl.css('overflow-x'),
                y: DOM.bodyEl.css('overflow-y')
            };
        }
    ...
    function _enableScroll() {
        if ($('.breaseModalDimmer:visible').length === 0 && this.dimmer) {
            ...
            DOM.bodyEl.css('overflow-x', _safe.x);
            DOM.bodyEl.css('overflow-y', _safe.y);
        }
    }
