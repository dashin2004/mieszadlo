define([
...
], function (PasswordPolicies, languageService) {
    ...
    p.viewPolicies = function () {
        if (this.policies && this.$policyBox) {
            this.$policyBox.find('.policyBoxHeader').text(languageService.getText('IAT/System/Dialog/POLICY_HEADER'));
            for (var key in this.policies) {
                //console.log('key:' + key);
                this.lines[key] = this.$policyBox.find('.' + key);
                if (key === 'minLength') {
                    var text = languageService.getText(_textKeys[key]);
                    text = text.replace('{1}', '' + this.policies[key]);
                    this.lines[key].find('p').text(text);
                } else {
                    this.lines[key].toggle(this.policies[key]);
                    this.lines[key].find('p').text(languageService.getText(_textKeys[key]));
                }
            }
        }
    };
