<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="file" select="'./ParameterTypeMappingIncludingTestEntries.xml'" />
  ...
  <xsl:template match="eb:EventBinding">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:message terminate="yes">Error: Invalid EventBinding. EventBinding with OpcUa-Event and OpcUa-Action is not supported.</xsl:message>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="convertTypeToScope">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:value-of select="'unknown target'" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="getContentScopeId">
    ...
    <xsl:if test="$widgetElements[@contentRefId != $firstFoundId]">
      <xsl:message terminate="yes">Error: Event source or targets use different contents. (first: "<xsl:value-of select="$firstFoundId" />", second: "<xsl:value-of select="$widgetElements[@contentRefId != $firstFoundId]/@contentRefId" />").Only one content can be used in one EventBinding.</xsl:message>
    </xsl:if>
    <xsl:value-of select="$firstFoundId" />
  </xsl:template>

  <xsl:template name="findOpcuaTarget">
    <xsl:param name="nodes" />
    <xsl:choose>
      <xsl:when test="$nodes[starts-with(@xsi:type, 'opcUa.NodeAction')]">
    ...
    <xsl:element name="{local-name()}">
      ...
      <xsl:for-each select="@*[name() !='xsi:type']">
        ...
        <xsl:if test="not($mappings[@parameter=$mappedKey]/@type)">
          <xsl:message terminate="yes">Error: Missing Parameter definition: <xsl:value-of select="$mappedKey" /></xsl:message>
      </xsl:if>
