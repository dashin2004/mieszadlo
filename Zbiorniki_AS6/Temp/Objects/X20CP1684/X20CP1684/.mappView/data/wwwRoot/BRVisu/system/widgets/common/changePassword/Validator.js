...
function (Message, languageService, StringUtils, PasswordPolicies) {

    /**
    * @class system.widgets.common.changePassword.Validator
    */
    ...
    * @return {system.widgets.common.changePassword.Message[]} return.arError Array of error messages.
    ...
    */
    p.validate = function (testInputs, form, userNameMinLength = 1) {
        let arError = [];
        let arInputs = [];
        let isValid = true;
        let policiesResult;
        ...
        testInputs.forEach(inputName => {
            let result = _validate.call(this, inputName, form, userNameMinLength);
            if (!result.isValid) {
                arError = arError.concat(result.messages);
                arInputs.push(inputName); 
            }
            policiesResult ??= result.policiesResult; 
